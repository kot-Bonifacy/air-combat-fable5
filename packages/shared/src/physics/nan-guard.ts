...
export function dumpPlaneState(state: PlaneState): string {
  ...
  return [
    ...
    `throttle=${v(state.throttle)} fuelFrac=${v(state.fuelFrac)} engineHeatFrac=${v(state.engineHeatFrac)} iasMs=${v(state.iasMs)} loadFactor=${v(state.loadFactor)} stalled=${String(state.stalled)}`,
...
export function validatePlaneState(state: PlaneState, context = ''): void {
  const fields: Record<string, number> = {
    ...
    engineHeatFrac: state.engineHeatFrac,
    iasMs: state.iasMs,
